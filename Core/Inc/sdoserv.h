...
typedef struct
  ...
  UINT16 Data[(((MAX_MBX_DATA_SIZE)
                - (DOWNLOAD_NORM_REQ_SIZE)) >> 1)]; /**< \brief Data buffer*/
}MBX_STRUCT_PACKED_END
...
typedef struct
  ...
  TCOEHEADER CoeHeader; /**< \brief CoE header*/
  TINITSDOHEADER SdoHeader; /**< \brief SDO header*/
  UINT16 Data[((MAX_EXPEDITED_DATA)
          >> 1)]; /**< \brief Data buffer*/
}MBX_STRUCT_PACKED_END
...
PROTO UINT8 (*pSdoPendFunc)(
        UINT16 Index,
        UINT8 Subindex,
        UINT32 Size,
        UINT16 MBXMEM *pData,
        UINT8 bCompleteAccess); /**< \brief Function pointer for the SDO pending request handler*/
/** @}*/

PROTO MEM_ADDR VARMEM aSdoInfoHeader[GET_MEM_SIZE(
        SDO_INFO_HEADER_BYTE_SIZE)]; /**< \brief SDO Info buffer*/
...
/*-----------------------------------------------------------------------------------------
 ...
 ------
 -----------------------------------------------------------------------------------------*/
PROTO UINT8 SDOS_SdoInfoInd(
        TSDOINFORMATION MBXMEM *pSdoInfoInd);

PROTO UINT8 SDOS_SdoInd(
        TINITSDOMBX MBXMEM *pSdoInd);

PROTO void SDOS_SdoRes(
        UINT8 abort,
        UINT32 objLength,
        UINT16 MBXMEM *pData);
