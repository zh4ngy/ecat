...
void PDO_OutputMapping(void) {
  ...
  HW_EscReadIsr(((MEM_ADDR *) aPdOutputData), nEscAddrOutputData,
                nPdOutputSize);
...
void HandleCycleTimeMeasurement(void) {
  if (!bDcSyncActive) //no DC sync configured (cycle time measurement 0x1C3x.2 is only available in no DC sync modes)
  {
    if (u32CycleTimeStartValue > 0) {
      ...
      if ((sSyncManInPar.u32CycleTime == 0)
  ...
  if (bEscIntEnabled) {
    ...
    if (ALEvent & PROCESS_OUTPUT_EVENT) {
      ...
      if (bEcatOutputUpdateRunning) {
        ...
        HW_EscReadByteIsr(u8dummy,
                          (nEscAddrOutputData + nPdOutputSize - 1));
      }
    }
...
void Sync0_Isr(void) {
  ...
  if (bDcSyncActive) {
      ...
      if (u16SmSync0Counter > u16SmSync0Value) {
        ...
        if ((nPdInputSize > 0) && (nPdOutputSize == 0)
            && (sSyncManInPar.u16SmEventMissedCounter
                <= sErrorSettings.u16SyncErrorCounterLimit)) {
          sSyncManInPar.u16SmEventMissedCounter =
                  sSyncManInPar.u16SmEventMissedCounter + 3;
        }
    ...
    if (bEcatInputUpdateRunning && (LatchInputSync0Value > 0)
