...
UINT8 MailboxServiceInd(TMBX MBXMEM *pMbx) {
  ...
  switch ((pMbx->MbxHeader.Flags[MBX_OFFS_TYPE] & MBX_MASK_TYPE)
          >> MBX_SHIFT_TYPE) {
    case MBX_TYPE_COE:
...
void MBX_MailboxWriteInd(TMBX MBXMEM *pMbx) {
  UINT8 result = 0;
  UINT8 mbxCounter = pMbx->MbxHeader.Flags[MBX_OFFS_COUNTER]
          >> MBX_SHIFT_COUNTER;
...
UINT8 MBX_MailboxSendReq(TMBX MBXMEM *pMbx, UINT8 flags) {
  ...
  pMbx->MbxHeader.Flags[MBX_OFFS_COUNTER] |= u8MbxReadCounter
          << MBX_SHIFT_COUNTER;
...
void MBX_CheckAndCopyMailbox(void) {
  ...
  if ((bSendMbxIsFull) /* a received mailbox service will not be processed
    ...
    HW_EscReadMbxMem((MEM_ADDR MBXMEM *) psWriteMbx, u16EscAddrReceiveMbx,
                     mbxLen);

    {
      ...
      if (smstate & SM_STATUS_MBX_BUFFER_FULL) {
        ...
        u8dummy = 0;
        HW_EscReadByte(u8dummy,
                       (u16EscAddrReceiveMbx + u16ReceiveMbxSize - 1));
...
UINT8 MBX_CopyToSendMailbox(TMBX MBXMEM *pMbx) {
  ...
  if (bSendMbxIsFull) {
    ...
    HW_EscWriteMbxMem((MEM_ADDR *) pMbx, u16EscAddrSendMbx,
                      (mbxSize + MBX_HEADER_SIZE));
    /* ECATCHANGE_HW(V5.10) HW1*/

    {
      /*Read Control and Status of SyncManager 1 to check if the buffer is still marked as empty*/
      VARVOLATILE UINT8 smstate = 0x00;
      HW_EscReadByte(smstate,
                     (ESC_SYNCMAN_STATUS_OFFSET + SIZEOF_SM_REGISTER));

      if (!(smstate & SM_STATUS_MBX_BUFFER_FULL)) {

        /*Write last Byte to trigger mailbox full flag*/
        u8dummy = 0;
        HW_EscWriteByte(u8dummy,
                        (u16EscAddrSendMbx + u16SendMbxSize - 1));
      }
    }
