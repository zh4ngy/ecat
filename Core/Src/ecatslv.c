...
TSYNCMAN ESCMEM *GetSyncMan(UINT8 channel) {
  HW_EscRead((MEM_ADDR *) &SyncManInfo,
             ESC_SYNCMAN_REG_OFFSET + (channel * SIZEOF_SM_REGISTER),
             SIZEOF_SM_REGISTER);
...
UINT8 CheckSmSettings(UINT8 maxChannel) {
  ...
  if (!(pSyncMan->Settings[SM_SETTING_ACTIVATE_OFFSET]
        & SM_SETTING_ENABLE_VALUE)) {
    /* receive mailbox is not enabled */
    result = ALSTATUSCODE_INVALIDMBXCFGINPREOP;
  } else if ((pSyncMan->Settings[SM_SETTING_CONTROL_OFFSET]
              & SM_SETTING_DIRECTION_MASK) != SM_SETTING_DIRECTION_WRITE_VALUE) {
    /* receive mailbox is not writable by the master*/
    result = ALSTATUSCODE_INVALIDMBXCFGINPREOP;
  } else if ((pSyncMan->Settings[SM_SETTING_CONTROL_OFFSET]
              & SM_SETTING_MODE_MASK) != SM_SETTING_MODE_ONE_BUFFER_VALUE) {
  ...
  if (result == 0) {
    ...
    if (!(pSyncMan->Settings[SM_SETTING_ACTIVATE_OFFSET]
      ...
      result = ALSTATUSCODE_INVALIDMBXCFGINPREOP;
                ...
                & SM_SETTING_DIRECTION_MASK) != SM_SETTING_DIRECTION_READ_VALUE) {
  ...
  if (result == 0 && maxChannel > PROCESS_DATA_IN) {
    ...
    if ((pSyncMan->Settings[SM_SETTING_ACTIVATE_OFFSET]
         & SM_SETTING_ENABLE_VALUE) != 0 && SMLength == 0) {
      /* the SM3 size is 0 and the SM3 is active */
      result = SYNCMANCHSETTINGS + 1;
    } else if (pSyncMan->Settings[SM_SETTING_ACTIVATE_OFFSET]
               & SM_SETTING_ENABLE_VALUE) {
      /* Sync Manager Channel 3 is active, input size has to greater 0 */
      if (SMLength
          != nPdInputSize || nPdInputSize == 0 || SMLength > MAX_PD_INPUT_SIZE) {
        /* sizes don't match */
        result = SYNCMANCHSIZE + 1;
      } else {
        /* sizes matches */
        if ((pSyncMan->Settings[SM_SETTING_CONTROL_OFFSET]
             & SM_SETTING_DIRECTION_MASK)
            == SM_SETTING_DIRECTION_READ_VALUE) {
          /* settings match */
          if (((nAlStatus == STATE_PREOP)
               && (SMAddress >= MIN_PD_READ_ADDRESS)
               && (SMAddress <= MAX_PD_READ_ADDRESS))
              || ((nAlStatus != STATE_PREOP)
                  && (SMAddress == nEscAddrInputData))) {
            /* addresses match */

            if ((pSyncMan->Settings[SM_SETTING_CONTROL_OFFSET]
                 & SM_SETTING_MODE_MASK)
                == SM_SETTING_MODE_ONE_BUFFER_VALUE) {
...
//    else
  if (result == 0 && maxChannel > PROCESS_DATA_OUT) {
    ...
    if ((pSyncMan->Settings[SM_SETTING_ACTIVATE_OFFSET]
      ...
      if (SMLength == nPdOutputSize && nPdOutputSize != 0
          && SMLength <= ((UINT16) MAX_PD_OUTPUT_SIZE)) {
        /* sizes match */
        if ((pSyncMan->Settings[SM_SETTING_CONTROL_OFFSET]
             & SM_SETTING_DIRECTION_MASK)
            == SM_SETTING_DIRECTION_WRITE_VALUE) {
          /* settings match */
          if (((nAlStatus == STATE_PREOP)
               && (SMAddress >= MIN_PD_WRITE_ADDRESS)
               && (SMAddress <= MAX_PD_WRITE_ADDRESS))
              || ((nAlStatus != STATE_PREOP)
                  && (SMAddress == nEscAddrOutputData))) {
            /* addresses match */
            {
              /* check, if watchdog trigger is enabled */
              if (pSyncMan->Settings[SM_SETTING_CONTROL_OFFSET]
                  & SM_SETTING_WATCHDOG_VALUE) {
                bWdTrigger = TRUE;
              } else {
                bWdTrigger = FALSE;
              }
...
UINT16 StartInputHandler(void) {
  ...
  if (((nEscAddrOutputData + nPdOutputSize * nPdOutputBuffer)
       > u16EscAddrSendMbx
       && (nEscAddrOutputData < (u16EscAddrSendMbx + u16SendMbxSize)))
   ...
   */
  if ((dcControl
       & (ESC_DC_SYNC_UNIT_ACTIVE_MASK | ESC_DC_SYNC_UNIT_AUTO_ACTIVE_MASK))
      != 0) {
    ...
    if (u16MinSuppSyncType != 0) {
      if ((((u16MinSuppSyncType & SYNCTYPE_DCSYNC0SUPP) == 0)
           && ((dcControl & ESC_DC_SYNC0_ACTIVE_MASK) != 0))
          || (((u16MinSuppSyncType & SYNCTYPE_DCSYNC1SUPP) == 0)
              && ((dcControl & ESC_DC_SYNC1_ACTIVE_MASK) != 0))) {
...

    {
      ...
      if (cycleTimeSync0 != 0
          && (cycleTimeSync0 < curMinCycleTime
              || cycleTimeSync0 > MAX_PD_CYCLE_TIME)) {
    ...
    if (bSubordinatedCycles
        && ((u16MinSuppSyncType & SYNCTYPE_SUBCYCLESUPP) == 0)) {
      return ALSTATUSCODE_DCINVALIDSYNCCFG;
    }
  }
  ...
  if (bSyncSetByUser) {
    if ((dcControl
         & (ESC_DC_SYNC_UNIT_ACTIVE_MASK
            | ESC_DC_SYNC_UNIT_AUTO_ACTIVE_MASK)) == 0) {
  ...
  if (!b3BufferMode) {
    ...
    if ((SyncType0x1C32 == SYNCTYPE_FREERUN)
        || (SyncType0x1C33 == SYNCTYPE_FREERUN)) {
  ...
  if ((SyncType0x1C32 == SYNCTYPE_DCSYNC0)
      || (SyncType0x1C32 == SYNCTYPE_DCSYNC1)
      || (SyncType0x1C33 == SYNCTYPE_DCSYNC0)
      || (SyncType0x1C33 == SYNCTYPE_DCSYNC1))/* Sync to Sync0 or Sync1 is enabled*/
  {
...
void AL_ControlInd(UINT8 alControl, UINT16 alStatusCode) {
  ...
  if (result == NOERROR_INWORK) {
    ...
    EsmTimeoutCounter -= (INT16) (EsmTimeoutCounter /
                                  10); //subtract 10% from the timeout to react before the master runs into a timeout.
    /*ECATCHANGE_END(V5.12) ESM3*/

  } else if (alControl != (nAlStatus & STATE_MASK)) {
    /* The slave state has changed */

    if ((result != 0 || alStatusCode != 0)
        && ((alControl | nAlStatus) & STATE_OP)) {
...
void DC_CheckWatchdog(void) {
  ...
  if (bDcSyncActive && bEcatInputUpdateRunning) {
    ...
    if (bDcRunning) {
      if (sSyncManOutPar.u16SmEventMissedCounter
...
void ECAT_StateChange(UINT8 alStatus, UINT16 alStatusCode) {
  ...
  if (bEcatWaitForAlControlRes) {
    ...
    if (alStatusCode != 0) {
      ...
      if (((nAlStatus & STATE_CHANGE) != STATE_CHANGE)
          || ((alStatus & STATE_MASK) < (nAlStatus & STATE_MASK))) {
...
void ECAT_Main(void) {
  ...
  if ((ALEventReg & SM_CHANGE_EVENT) && !bEcatWaitForAlControlRes
      && (nAlStatus & STATE_CHANGE) == 0
      && (nAlStatus & ~STATE_CHANGE) != STATE_INIT) {
  ...
  if (bMbxRunning) {
    ...
    if (((sm1Activate & SM_SETTING_REPAET_REQ_MASK) && !bMbxRepeatToggle)
        || (!(sm1Activate & SM_SETTING_REPAET_REQ_MASK)
            && bMbxRepeatToggle)) {
      ...
      HW_EscWriteByte(sm1Activate,
                      (ESC_SM_PDICONTROL_OFFSET + SIZEOF_SM_REGISTER));
    }
