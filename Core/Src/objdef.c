...
/*---------------------------------------------------------------------------------------
 ...
 ------
 ---------------------------------------------------------------------------------------*/
const UINT16 cBitMask[16] = {0x0000, 0x0001, 0x0003, 0x0007, 0x000F, 0x001F,
...
OBJCONST TOBJECT OBJMEM *OBJ_GetObjectHandle(UINT16 index) {
  OBJCONST TOBJECT OBJMEM *pObjEntry =
          (OBJCONST TOBJECT OBJMEM *) COE_GetObjectDictionary();
 ...
 *////////////////////////////////////////////////////////////////////////////////////////
UINT32 OBJ_GetObjectLength(UINT16 index, UINT8 subindex,
                           OBJCONST TOBJECT OBJMEM *pObjEntry, UINT8 bCompleteAccess) {
  /* get the information of ObjCode and MaxSubindex in local variables to support different types of microcontroller */
  UINT8 objCode = (pObjEntry->ObjDesc.ObjFlags & OBJFLAGS_OBJCODEMASK)
          >> OBJFLAGS_OBJCODESHIFT;

  /*ECATCHANGE_START(V5.12) COE6*/
  UINT8 maxSubindex = (pObjEntry->ObjDesc.ObjFlags & OBJFLAGS_MAXSUBINDEXMASK)
    ...
    if (pObjEntry->Index >= 0x1000) {
      ...
      if (t) {
        ...
        while (t && i <= maxSubindex) {
          if (OBJ_GetEntryDesc(pObjEntry, (UINT8) i)->ObjAccess
              & listFlags) {
            t = 0;
          }
          i++;
 ...
 *////////////////////////////////////////////////////////////////////////////////////////
UINT16 OBJ_GetObjectList(UINT16 listType, UINT16 *pIndex, UINT16 size,
                         UINT16 MBXMEM *pData, UINT8 *pAbort) {
  /* the variable listFlags contains the mask used for the ObjAccess in the Entry-Desc
   see the structure TSDOINFOENTRYDESC in sdoserv.h, listType = 0 indicates that
   all objects has to be counted */
  UINT16 listFlags = 0x0020 << listType;
    ...
    while (pObjEntry != NULL && size > 1) {
      ...
      if (pObjEntry->Index >= 0x1000) {
        ...
        if (t) {
          ...
          while (t && i <= maxSubindex) {
            if (OBJ_GetEntryDesc(pObjEntry, i)->ObjAccess
                & listFlags) {
              t = 0;
            }
            i++;
 ...
 *////////////////////////////////////////////////////////////////////////////////////////
UINT16 OBJ_GetDesc(UINT16 index, UINT8 subindex,
    ...
    if (objCode == OBJCODE_REC) {
      {
...

        {
          ...
          OBJCONST UCHAR OBJMEM *pSubDesc =
                  (OBJCONST UCHAR OBJMEM *) OBJGETNEXTSTR(pDesc);
          ...
          {
            if (i == tmpSubindex) {
              strSize = (UINT16) OBJSTRLEN(
                      (OBJCONST CHAR OBJMEM *) pSubDesc);
              if (pData && strSize) {
              ...
              i++;
              pSubDesc = (OBJCONST UCHAR OBJMEM *) OBJGETNEXTSTR(
                      pSubDesc);
            }
          }
        }
 ...
 *////////////////////////////////////////////////////////////////////////////////////////
OBJCONST TSDOINFOENTRYDESC OBJMEM *OBJ_GetEntryDesc(
        OBJCONST TOBJECT OBJMEM *pObjEntry, UINT8 Subindex) {
 ...
 *////////////////////////////////////////////////////////////////////////////////////////
OBJCONST TSDOINFOOBJDESC OBJMEM *OBJ_GetObjDesc(
        OBJCONST TOBJECT OBJMEM *pObjEntry) {
...
UINT8 CheckSyncTypeValue(UINT16 index, UINT16 NewSyncType) {
  switch (NewSyncType) {
    ...
    case SYNCTYPE_SM_SYNCHRON:
      if ((index == 0x1C32) && (nPdOutputSize > 0)
          && ((sSyncManOutPar.u16SyncTypesSupported
               & SYNCTYPE_SYNCHRONSUPP) > 0)) {
        /*SyncManager sync mode is supported and output process data is configured*/
        return 0;
      } else if ((index == 0x1C33)
                 && ((sSyncManInPar.u16SyncTypesSupported & SYNCTYPE_SYNCHRONSUPP)
                     > 0) && (nPdOutputSize == 0) && (nPdInputSize > 0)) {
    ...
    case SYNCTYPE_DCSYNC0:
      if ((index == 0x1C32)
          && ((sSyncManOutPar.u16SyncTypesSupported & SYNCTYPE_DCSYNC0SUPP)
              > 0)) {
        return 0;
      } else if ((index == 0x1C33)
                 && ((sSyncManInPar.u16SyncTypesSupported & SYNCTYPE_DCSYNC0SUPP)
                     > 0)) {
        return 0;
      }
      break;
    ...
    case SYNCTYPE_DCSYNC1:
      if ((index == 0x1C32)
          && ((sSyncManOutPar.u16SyncTypesSupported & SYNCTYPE_DCSYNC1SUPP)
              > 0)) {
        return 0;
      } else if ((index == 0x1C33)
                 && ((sSyncManInPar.u16SyncTypesSupported & SYNCTYPE_DCSYNC1SUPP)
                     > 0)) {
        return 0;
      }
      break;
 ...
 *////////////////////////////////////////////////////////////////////////////////////////
UINT8 OBJ_Read(UINT16 index, UINT8 subindex, UINT32 objSize,
               OBJCONST TOBJECT OBJMEM *pObjEntry, UINT16 MBXMEM *pData,
  ...
  UINT8 objCode = (pObjEntry->ObjDesc.ObjFlags & OBJFLAGS_OBJCODEMASK)
          >> OBJFLAGS_OBJCODESHIFT;
  UINT16 maxSubindex =
          (pObjEntry->ObjDesc.ObjFlags & OBJFLAGS_MAXSUBINDEXMASK)
                  >> OBJFLAGS_MAXSUBINDEXSHIFT;
  OBJCONST TSDOINFOENTRYDESC OBJMEM *pEntry;
  ...
  if (bCompleteAccess) {
    ...
    if (pEntry->ObjAccess != 0x0) {
      ...
      if (0
          == (((UINT8) ((pEntry->ObjAccess & ACCESS_READ) << 1))
          ...
          if (i == 0 && objCode != OBJCODE_VAR) {
            ...
            switch (dataType) {
              ...
              case DEFTYPE_BYTE: {
                ...
                bitMask = cBitMask[pEntry->BitLength]
                        << (bitOffset & 0x0F);
              ...
              case DEFTYPE_ARRAY_OF_UDINT:
                ...
                OBJTOMBXMEMCPY(pData, pVarPtr,
                               BIT2BYTE(pEntry->BitLength));
 ...
 *////////////////////////////////////////////////////////////////////////////////////////
UINT8 OBJ_Write(UINT16 index, UINT8 subindex, UINT32 dataSize,
                OBJCONST TOBJECT OBJMEM *pObjEntry, UINT16 MBXMEM *pData,
                UINT8 bCompleteAccess) {
  UINT16 i = subindex;
  /* get the information of ObjCode and MaxSubindex in local variables to support different types of microcontroller */
  ...
  UINT16 maxConfiguredSubindex = (pObjEntry->ObjDesc.ObjFlags
                                  & OBJFLAGS_MAXSUBINDEXMASK)
          >> OBJFLAGS_MAXSUBINDEXSHIFT; //required to check if value for Subindex0 is valid
  OBJCONST TSDOINFOENTRYDESC OBJMEM *pEntry;
  ...
  if (bCompleteAccess) {
    ...
    /*ECATCHANGE_START(V5.12) COE3*/
    if (0
        == (((UINT8) ((pEntry->ObjAccess & ACCESS_WRITE) >> 2))
            & (nAlStatus & STATE_MASK)))
      /*ECATCHANGE_END(V5.12) COE3*/
    {
  ...
  if (pObjEntry->Write != NULL) {
    ...
    return pObjEntry->Write(index, subindex, dataSize, pData,
                            bCompleteAccess);
  } else {
    UINT8 bWritten = 0;
            ...
            switch (dataType) {
              ...
              case DEFTYPE_WORD:
...

                {
                  ...
                  {
                    if (!IS_RX_PDO(u16NewData)
                        && !IS_TX_PDO(u16NewData)
                        && (u16NewData != 0)) //check if the new assign entry value is valid
                    {
                      return ABORTIDX_VALUE_EXCEEDED;
                    }
                  }
                  ...
                  if (index == 0x1C32 || index == 0x1C33) {
                    if (i == 1) /* "Synchronisation type" written */
                    {
                      ...
                      if (pVarPtr[0] != u16NewData) {
                        result = CheckSyncTypeValue(index,
                                                    u16NewData);
                    ...
                    if (i == 8) /* "Get Cycle Time" written*/
                    {
                      ...
                      sSyncManOutPar.u32CalcAndCopyTime =
                              (PD_OUTPUT_CALC_AND_COPY_TIME);
                      sSyncManOutPar.u32MinCycleTime =
                              (MIN_PD_CYCLE_TIME);
                      sSyncManOutPar.u32CycleTime = 0;

                      sSyncManInPar.u32CalcAndCopyTime =
                              (PD_INPUT_CALC_AND_COPY_TIME);
                      sSyncManInPar.u32MinCycleTime =
                              (MIN_PD_CYCLE_TIME);
