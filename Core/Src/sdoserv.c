...
extern UINT16 OBJ_GetDesc(UINT16 index, UINT8 subindex,
                          OBJCONST TOBJECT OBJMEM *pObjEntry, UINT16 MBXMEM *pData);

extern OBJCONST TSDOINFOENTRYDESC OBJMEM *OBJ_GetEntryDesc(
        OBJCONST TOBJECT OBJMEM *pObjEntry, UINT8 Subindex);

extern OBJCONST TSDOINFOOBJDESC OBJMEM *OBJ_GetObjDesc(
...
static UINT8 SdoDownloadSegmentInd(TDOWNLOADSDOSEGREQMBX MBXMEM *pSdoInd) {
  ...
  if (SWAPWORD(pSdoInd->SdoHeader.SegHeader & SEGHEADER_TOGGLE)
      == bSdoSegLastToggle)
    /* ECATCHANGE_END(V5.12) ECAT2*/
  {
    ...
    UINT16 maxData = u16ReceiveMbxSize - MBX_HEADER_SIZE
                     - SEGMENT_NORM_HEADER_SIZE;
    /* the new toggle bit is stored in bSdoSegLastToggle */
    /* ECATCHANGE_START(V5.12) ECAT2*/
    bSdoSegLastToggle = SWAPWORD(
            pSdoInd->SdoHeader.SegHeader & SEGHEADER_TOGGLE);
    /* ECATCHANGE_END(V5.12) ECAT2*/
    ...
    if (pSdoSegData) {
      ...
      if (pSdoInd->SdoHeader.SegHeader & SEGHEADER_NOMOREFOLLOWS) {
        ...
        if (bytesToSave <= maxData) {
          ...
          if (((bytesToSave
                <= (UINT32) (mbxSize - SEGMENT_NORM_HEADER_SIZE))
               && (bytesToSave
                   == ((UINT16) (MIN_SEGMENTED_DATA
                                 - ((pSdoInd->SdoHeader.SegHeader
      ...
      if (abort == 0) {
        ...
        MBXMEMCPY(((UINT8 *) pSdoSegData) + (nSdoSegBytesToHandle),
                  pSdoInd->SdoHeader.Data, bytesToSave);

        if (bSdoSegFollows == FALSE) {
          /* it was the last segment, OBJ_Write will called to make the Write-operation */
          abort = OBJ_Write(nSdoSegIndex, nSdoSegSubindex,
                            nSdoSegCompleteSize, pSdoSegObjEntry,
                            (UINT16 MBXMEM *) pSdoSegData, bSdoSegAccess);
  ...
  if (abort == 0) {
    ...
    pSdoInd->CoeHeader |= ((UINT16) COESERVICE_SDORESPONSE)
            << COEHEADER_COESERVICESHIFT;
    /* the SDO Download Segment header depends if it was the last segment or not */
    /* ECATCHANGE_START(V5.12) ECAT2*/
    if (bSdoSegLastToggle) {
      pSdoInd->SdoHeader.SegHeader = SWAPWORD(
              SDOSERVICE_DOWNLOADSEGMENTRES | SEGHEADER_TOGGLE);
    } else {
      pSdoInd->SdoHeader.SegHeader = SWAPWORD(
              SDOSERVICE_DOWNLOADSEGMENTRES);
    }
    /* ECATCHANGE_END(V5.12) ECAT2*/
...
static UINT8 SdoUploadSegmentInd(TUPLOADSDOSEGREQMBX MBXMEM *pSdoInd) {
  UINT8 abort = 0;
  TUPLOADSDOSEGRESMBX MBXMEM *pSdoSegRes =
          (TUPLOADSDOSEGRESMBX MBXMEM *) pSdoInd;
    ...
    /* ECATCHANGE_END(V5.12) ECAT2*/
  {
    ...
    {
      maxData = u16SendMbxSize - MBX_HEADER_SIZE
                - SEGMENT_NORM_HEADER_SIZE;
    }
    ...
    MBXMEMCPY(pSdoSegRes->SdoHeader.Data,
              &(((UINT8 *) pSdoSegData)[nSdoSegBytesToHandle]), size);

    /* the SDO Upload Segment header depends if there is still data to be sent */
    pSdoSegRes->CoeHeader &= ~COEHEADER_COESERVICEMASK;
    pSdoSegRes->CoeHeader |= ((UINT16) COESERVICE_SDORESPONSE)
            << COEHEADER_COESERVICESHIFT;
    ...
    /* ECATCHANGE_START(V5.12) ECAT2*/
    if (bSdoSegFollows) {
      pSdoSegRes->SdoHeader.SegHeader |= SWAPWORD(
              SDOSERVICE_UPLOADSEGMENTRES | bSdoSegLastToggle);
    } else {
      pSdoSegRes->SdoHeader.SegHeader |= SWAPWORD(
              SDOSERVICE_UPLOADSEGMENTRES | bSdoSegLastToggle
    ...
    if (size < MIN_SEGMENTED_DATA) {
      ...
      pSdoSegRes->SdoHeader.SegHeader |= (MIN_SEGMENTED_DATA - size)
              << SEGHEADERSHIFT_SEGDATASIZE;
    } else {
      pSdoSegRes->MbxHeader.Length = ((UINT16) size)
                                     + SEGMENT_NORM_HEADER_SIZE;
    }
 ...
 *////////////////////////////////////////////////////////////////////////////////////////
void SdoRes(UINT8 abort, UINT8 command, UINT8 completeAccess, UINT16 dataSize,
            UINT32 objLength, TINITSDOMBX MBXMEM *pSdoRes) {
  /* for an upload segment response the toggle bit was overwritten */
  if ((command != SDOSERVICE_UPLOADSEGMENTREQ)
      && (command != SDOSERVICE_DOWNLOADSEGMENTREQ)) {
  ...
  if (abort == 0) {
    ...
    pSdoRes->CoeHeader |= ((UINT16) COESERVICE_SDORESPONSE)
            << COEHEADER_COESERVICESHIFT;
    if (command == SDOSERVICE_INITIATEUPLOADREQ) {
      ...
      if ((objLength <= 4) && (objLength > 0)) {
        ...
        pSdoRes->SdoHeader.Sdo[SDOHEADER_COMMANDOFFSET] |=
                SDOHEADER_SIZEINDICATOR |
                SDOHEADER_TRANSFERTYPE | completeAccess
                | ((MAX_EXPEDITED_DATA - ((UINT8) objLength))
                        << SDOHEADERSHIFT_DATASETSIZE) |
        ...
        if (dataSize < objLength) {
          ...
          pSdoRes->MbxHeader.Length = UPLOAD_NORM_RES_SIZE
                                      + ((UINT16) objLength);
        }
        ((TINITSDOUPLOADNORMRESMBX MBXMEM *) pSdoRes)->CompleteSize[0] =
                SWAPWORD((UINT16) objLength);
        ((TINITSDOUPLOADNORMRESMBX MBXMEM *) pSdoRes)->CompleteSize[1] =
                SWAPWORD((UINT16)(objLength >> 16));
        pSdoRes->SdoHeader.Sdo[SDOHEADER_COMMANDOFFSET] |=
                SDOHEADER_SIZEINDICATOR | completeAccess |
                SDOSERVICE_INITIATEUPLOADRES;
      ...
      pSdoRes->MbxHeader.Length = DOWNLOAD_NORM_RES_SIZE;
      pSdoRes->SdoHeader.Sdo[SDOHEADER_COMMANDOFFSET] |=
              SDOSERVICE_INITIATEDOWNLOADRES;
    }
  } else {
    /* generate a SDO-Abort-Request */
    pSdoRes->MbxHeader.Length = ABORT_NORM_RES_SIZE;
    pSdoRes->CoeHeader &= ~COEHEADER_COESERVICEMASK;
    pSdoRes->CoeHeader |= ((UINT16) COESERVICE_SDOREQUEST)
            << COEHEADER_COESERVICESHIFT;
    /* ECATCHANGE_START(V5.12) COE2*/
    pSdoRes->SdoHeader.Sdo[SDOHEADER_COMMANDOFFSET] |=
            SDOSERVICE_ABORTTRANSFER;
    /* ECATCHANGE_END(V5.12) COE2*/

    ((TABORTSDOTRANSFERREQMBX MBXMEM *) pSdoRes)->AbortCode = SWAPDWORD(
            cAbortCode[abort]);
  }
...
UINT8 SDOS_SdoInd(TINITSDOMBX MBXMEM *pSdoInd) {
  ...
  switch (command) {
    ...
    case SDOSERVICE_INITIATEUPLOADREQ:
      ...
      index = ((UINT16) (pSdoInd->SdoHeader.Sdo[SDOHEADER_INDEXHIOFFSET]
      ...
      if (pObjEntry) {
...

        {
          ...
          UINT8 maxSubindex = (pObjEntry->ObjDesc.ObjFlags
                               & OBJFLAGS_MAXSUBINDEXMASK) >> OBJFLAGS_MAXSUBINDEXSHIFT;

          if (subindex > maxSubindex) {
            abort = ABORTIDX_SUBINDEX_NOT_EXISTING;
          } else {
            dataSize = objLength = OBJ_GetObjectLength(index, subindex,
                                                       pObjEntry,
                                                       (UINT8) (sdoHeader & SDOHEADER_COMPLETEACCESS));
          }
            ...
            /*ECATCHANGE_END(V5.12) COE6*/
          {
            if (command == SDOSERVICE_INITIATEUPLOADREQ) {
              ...
              if ((objLength <= MAX_EXPEDITED_DATA)
                ...
                pData =
                        (UINT16 MBXMEM *) ((TINITSDOUPLOADEXPRESMBX MBXMEM *) pSdoInd)->Data;
                ...
                dataSize = u16SendMbxSize - MBX_HEADER_SIZE
                           - UPLOAD_NORM_RES_SIZE;
                if (dataSize < objLength) {
                  /* Segmented Upload */
                  segTransfer = 1;
                } else {
                  ...
                  /* ECATCHANGE_START(V5.12)*/
                  pData =
                          (UINT16 MBXMEM *) ((TINITSDOUPLOADNORMRESMBX MBXMEM *) pSdoInd)->Data;
                  /* ECATCHANGE_END(V5.12)*/
                }
              }
            } else {
          ...
          /*ECATCHANGE_END(V5.12) COE6*/
        {
          ...
          if (bTransferType) {
            ...
            UINT32 downloadSize =
                    ((UINT32) (SWAPWORD(
                            ((TINITSDODOWNLOADNORMREQMBX MBXMEM *) pSdoInd)->CompleteSize[1]))
                            << 16)
                    + (SWAPWORD(
                            ((TINITSDODOWNLOADNORMREQMBX MBXMEM *) pSdoInd)->CompleteSize[0]));

            /* HBu 29.03.06: if it is a segmented download the mbxSize has to be the complete mailbox size */
            if ((MBX_HEADER_SIZE + EXPEDITED_FRAME_SIZE + downloadSize)
                > u16ReceiveMbxSize) {
            ...
            /* pData is the pointer to the downloaded object data */
            pData =
                    (UINT16 MBXMEM *) ((TINITSDODOWNLOADNORMREQMBX MBXMEM *) pSdoInd)->Data;
            ...
            if (dataSize
                > (UINT32) (mbxSize - DOWNLOAD_NORM_REQ_SIZE)) {
              /* Segmented Download */
              segTransfer = 1;
            }
          }
        ...
        if (abort == 0) {
          if (segTransfer) {
            ...
            pSdoSegData = (UINT16 VARMEM *) ALLOCMEM(
                    ROUNDUPBYTE2WORD(nSdoSegCompleteSize));
            ...
            if (pSdoSegData == NULL) {
              ...
              if (command == SDOSERVICE_INITIATEUPLOADREQ) {
                /* Segmented Upload */
                abort = OBJ_Read(index, subindex, objLength,
                                 pObjEntry, (UINT16 MBXMEM *) pSdoSegData,
                                 bCompleteAccess);
                if (abort == 0) {
                  /* ECATCHANGE_START(V5.12)*/
                  MBXMEMCPY(
                          (UINT16 *) ((TINITSDOUPLOADNORMRESMBX MBXMEM *) pSdoInd)->Data,
                          pSdoSegData, dataSize);
                  /* ECATCHANGE_END(V5.12)*/
                  ...
                  pSdoResStored->SdoHeader.Sdo[SDOHEADER_COMMANDOFFSET] &=
                          ~SDOHEADER_COMMANDMASK;
                  pSdoResStored->SdoHeader.Sdo[SDOHEADER_COMMANDOFFSET] |=
                          (sdoHeader
                           & (SDOHEADER_COMPLETEACCESS
                              | SDOHEADER_COMMAND));
                  nSdoSegService = SDOSERVICE_UPLOADSEGMENTREQ;
                  return 0;
                }
                ...
                /* Segmented Download */
                /* ECATCHANGE_START(V5.12)*/
                MBXMEMCPY(pSdoSegData,
                          (UINT16 *) ((TINITSDODOWNLOADNORMREQMBX MBXMEM *) pSdoInd)->Data,
                          mbxSize - DOWNLOAD_NORM_REQ_SIZE);
                /* ECATCHANGE_END(V5.12)*/
            ...
            if (command == SDOSERVICE_INITIATEUPLOADREQ) {
              ...
              abort = OBJ_Read(index, subindex, objLength, pObjEntry,
                               pData, bCompleteAccess);
              if (abort == ABORTIDX_WORKING) {
                /* the application generates the SDO-Response later on by calling SDOS_SdoRes (only possible if object access function pointer is defined) */
              ...
              abort = OBJ_Write(index, subindex, dataSize, pObjEntry,
                                pData, bCompleteAccess);
              if (abort == ABORTIDX_WORKING) {
                /* the application generates the SDO-Response later on by calling SDOS_SdoRes (only possible if object access function pointer is defined) */
    ...
    case SDOSERVICE_UPLOADSEGMENTREQ:
      if (command == nSdoSegService) {
        if (command == SDOSERVICE_DOWNLOADSEGMENTREQ) {
          abort = SdoDownloadSegmentInd(
                  (TDOWNLOADSDOSEGREQMBX MBXMEM *) pSdoInd);
        } else {
          abort = SdoUploadSegmentInd(
                  (TUPLOADSDOSEGREQMBX MBXMEM *) pSdoInd);
        }
      } else {
        abort = ABORTIDX_COMMAND_SPECIFIER_UNKNOWN;
      }
  ...
  if (abort != ABORTIDX_WORKING) {
    ...
    SdoRes(abort, command, (UINT8) (sdoHeader & SDOHEADER_COMPLETEACCESS),
           (UINT16) dataSize, objLength, pSdoInd);
  }
...
void SDOS_SdoRes(UINT8 abort, UINT32 objLength, UINT16 MBXMEM *pData) {
  ...
  if (bSdoInWork) {
    ...
    UINT8 command = pSdoResStored->SdoHeader.Sdo[SDOHEADER_COMMANDOFFSET]
                    & SDOHEADER_COMMAND;
    UINT8 completeAccess =
            pSdoResStored->SdoHeader.Sdo[SDOHEADER_COMMANDOFFSET]
            & SDOHEADER_COMPLETEACCESS;
    ...
    if (command == SDOSERVICE_INITIATEUPLOADREQ) {
      ...
      if (dataSize < objLength) {
        ...
        /* ECATCHANGE_START(V5.12)*/
        MBXMEMCPY(
                (UINT16 *) ((TINITSDOUPLOADNORMRESMBX MBXMEM *) pSdoResStored)->Data,
                pData, dataSize);
        /* ECATCHANGE_END(V5.12)*/
        nSdoSegBytesToHandle = dataSize;
        ...
        /* Expedited response */
        if (pData
            != ((TINITSDOUPLOADEXPRESMBX MBXMEM *) pSdoResStored)->Data) {
          /* the data is not in the response buffer yet, it shall be copied */
          /* ECATCHANGE_START(V5.12)*/
          MBXMEMCPY(
                  (UINT16 *) ((TINITSDOUPLOADEXPRESMBX MBXMEM *) pSdoResStored)->Data,
                  pData, objLength);
          /* ECATCHANGE_END(V5.12)*/
        }
      } else {
        /* Normal response */
        if (pData
            != ((TINITSDOUPLOADNORMRESMBX MBXMEM *) pSdoResStored)->Data) {
          /* the data is not in the response buffer yet, it shall be copied */
          /* ECATCHANGE_START(V5.12)*/
          MBXMEMCPY(
                  (UINT16 *) ((TINITSDOUPLOADNORMRESMBX MBXMEM *) pSdoResStored)->Data,
    ...
    bSdoInWork = FALSE;
    SdoRes(abort, command, completeAccess, dataSize, objLength,
           pSdoResStored);
  }
}
...
void SODS_ClearPendingResponse() {
  if ((bSdoInWork == TRUE) && (pSdoResStored != NULL)
      && (((TMBX MBXMEM *) pSdoResStored) != psRepeatMbx)
      && (((TMBX MBXMEM *) pSdoResStored) != psStoreMbx)) {
...
UINT8 SDOS_SdoInfoInd(TSDOINFORMATION MBXMEM *pSdoInfoInd) {
  ...
  UINT8 opCode = (UINT8) ((pSdoInfoInd->SdoHeader.InfoHead
                           & INFOHEAD_OPCODE_MASK) >> INFOHEAD_OPCODE_SHIFT);
  OBJCONST TOBJECT OBJMEM *pObjEntry;
  UINT16 index;
  ...
  switch (opCode) {
    case SDOINFOSERVICE_OBJDICTIONARYLIST_Q:
      ...
      if (SWAPWORD(
                  pSdoInfoInd->SdoHeader.Data.List.ListType) <= INFO_LIST_TYPE_MAX) {
        UINT16 size = 0;
        /* the variable listType contains the requested listType */
        UINT8 listType = (UINT8) SWAPWORD(
                pSdoInfoInd->SdoHeader.Data.List.ListType);
        ...
        if (listType-- == 0) {
          ...
          for (i = 0; i < INFO_LIST_TYPE_MAX; i++) {
            ...
            ((UINT16 MBXMEM *) &pSdoInfoInd->CoeHeader)[(SIZEOF_SDOINFOLISTSTRUCT
                    >> 1) + i] = SWAPWORD(n);
          }
          ...
          if (nSdoInfoFragmentsLeft) {
            ...
            {
              size =
                      u16SendMbxSize - SIZEOF_SDOINFO
                      - MBX_HEADER_SIZE;
            }
            /* initialize pData with the pointer where the fragment has to be copied */
            pData =
                    &((UINT16 MBXMEM *) &pSdoInfoInd->CoeHeader)[SIZEOF_SDOINFO
                            >> 1];
            ...
            /* initialize size with the maximum size fits into one mailbox service */
            {
              size = u16SendMbxSize - SIZEOF_SDOINFOLISTSTRUCT
                     - MBX_HEADER_SIZE;
            }
            /* initialize pData with the pointer where the fragment has to be copied */
            pData =
            ...
            if ((n << 1) > size) {
              ...
              nSdoInfoFragmentsLeft = (((n << 1) - size
                                        + (Fragsize - 1)) / Fragsize);
            } else {
              nSdoInfoFragmentsLeft = 0;
            }
        ...
        if (abort == 0) {
          ...
          pSdoInfoInd->SdoHeader.InfoHead |=
                  (UINT16) (SDOINFOSERVICE_OBJDICTIONARYLIST_S
                          << INFOHEAD_OPCODE_SHIFT);
          /* number of fragments still has to be sent */
          pSdoInfoInd->SdoHeader.FragmentsLeft = SWAPWORD(
                  nSdoInfoFragmentsLeft);
          ...
          if (nSdoInfoFragmentsLeft) {
            ...
            pSdoInfoInd->SdoHeader.InfoHead |=
                    (UINT16) (SDOINFOSERVICE_INCOMPLETE
                            << INFOHEAD_OPCODE_SHIFT);
      ...
      if (index < 0x1000) {
        ...
        if (pObjEntry) {
          ...
          if (opCode == SDOINFOSERVICE_OBJDESCRIPTION_Q) {
            ...
            OBJTOMBXMEMCPY(&pSdoInfoInd->SdoHeader.Data.Obj.Res,
                           OBJ_GetObjDesc(pObjEntry), SDO_INFO_OBJ_DESC_SIZE);
            ...
            size = OBJ_GetDesc(index, 0, pObjEntry,
                               NULL) + SIZEOF_SDOINFOOBJSTRUCT;
            ...
            if (size > (u16SendMbxSize - MBX_HEADER_SIZE)) {
              ...
              size =
                      OBJ_GetDesc(index, 0, pObjEntry,
                                  ((UINT16 MBXMEM *) &(&pSdoInfoInd->SdoHeader.Data.Obj.Res)[1])) +
                      SIZEOF_SDOINFOOBJSTRUCT;
            }
          } else {
            /* entry description is requested,
             get the requested subindex */
            UINT8 subindex =
                    (UINT8) ((pSdoInfoInd->SdoHeader.Data.Entry.Info
                              & ENTRY_MASK_SUBINDEX)
                            >> ENTRY_SUBINDEX_SHIFT);

            /* get the maximum subindex */
            UINT8 maxSubindex = (OBJ_GetObjDesc(pObjEntry)->ObjFlags
            ...
            if (subindex <= maxSubindex) {
              ...
              OBJTOMBXMEMCPY(&pSdoInfoInd->SdoHeader.Data.Entry.Res,
                             OBJ_GetEntryDesc(pObjEntry, subindex),
                             SIZEOF(TSDOINFOENTRYDESC));

              /* the transmission of the value info is not supported yet of the sample code */
              pSdoInfoInd->SdoHeader.Data.Entry.Info &=
                      ~ENTRY_MASK_VALUEINFO;
              ObjectFlags = OBJ_GetObjDesc(pObjEntry)->ObjFlags;
              ObjectFlags = (ObjectFlags & OBJFLAGS_OBJCODEMASK)
                      >> OBJFLAGS_OBJCODESHIFT;

              if (((ObjectFlags == OBJCODE_ARR)
                   || (ObjectFlags == OBJCODE_REC))
                  && (subindex == 0)) {
                OBJTOMBXSTRCPY(
                        ((UINT16 MBXMEM *) &(&pSdoInfoInd->SdoHeader.Data.Entry.Res)[1]),
                        aSubindexDesc, SIZEOF(aSubindexDesc));
                ...
                size = OBJ_GetDesc(index, subindex, pObjEntry,
                                   NULL) + SIZEOF_SDOINFO + SIZEOF(TSDOINFOENTRY);
                if (size > (u16SendMbxSize - MBX_HEADER_SIZE)) {
                  /* size of the object description does not fit in the mailbox,
                   the object description will be sent without the name */
                  ...
                  size =
                          OBJ_GetDesc(index, subindex, pObjEntry,
                                      ((UINT16 MBXMEM *) &(&pSdoInfoInd->SdoHeader.Data.Entry.Res)[1])) +
                          SIZEOF_SDOINFO + SIZEOF(TSDOINFOENTRY);
                }
              }
            } else {
              abort = ABORTIDX_SUBINDEX_NOT_EXISTING;
          ...
          if (abort == 0) {
            {
              ...
              pSdoInfoInd->SdoHeader.InfoHead |=
                      (UINT16) ((opCode + 1) << INFOHEAD_OPCODE_SHIFT);
            }
          }
        } else {
          abort = ABORTIDX_OBJECT_NOT_EXISTING;
  ...
  if (abort) {
    ...
    pSdoInfoInd->SdoHeader.InfoHead &= ~INFOHEAD_OPCODE_MASK;
    pSdoInfoInd->SdoHeader.InfoHead |= (UINT16) ((SDOINFOSERVICE_ERROR_Q)
            << INFOHEAD_OPCODE_SHIFT);

    pSdoInfoInd->SdoHeader.FragmentsLeft = 0;
    pSdoInfoInd->SdoHeader.Data.Error.ErrorCode = SWAPDWORD(
            cAbortCode[abort]);
